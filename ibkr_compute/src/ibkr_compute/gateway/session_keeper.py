...
class SessionKeeper:
    def __init__(self, gateway_url: str = None, pb_client=None,
        ...
        self._last_session_expired_callback: Optional[float] = None
        ...
        while self._running:
            ...
            if "error" not in tickle_result:
                ...
                if not auth.get("authenticated", False):
                    logger.warning("Session not authenticated, manual 2FA required")
                    self._authenticated = False
                    self._log_to_pb("reauth_trigger", "warning", "Session expired; waiting manual 2FA trigger")
                    now = time.time()
                    should_callback = (
                        self._last_session_expired_callback is None
                        or (now - self._last_session_expired_callback) >= CALLBACK_COOLDOWN_SECONDS
                    )
                    if should_callback and self.on_session_expired:
                        self._last_session_expired_callback = now
                ...
                now = time.time()
                should_callback = (
                    self._last_gateway_down_callback is None
                    or (now - self._last_gateway_down_callback) >= CALLBACK_COOLDOWN_SECONDS
                )
                if should_callback and self.on_gateway_down:
                    self._last_gateway_down_callback = now
