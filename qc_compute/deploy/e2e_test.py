...
在远端执行:
  source /opt/qc_compute/.env
  PYTHONPATH=/opt/qc_compute/src python3 /opt/qc_compute/deploy/e2e_test.py
"""
...
def qc_get(path):
    return requests.get(QC_URL + path, timeout=10)


def qc_post(path, data=None):
    return requests.post(QC_URL + path, json=data or {}, timeout=15)
...
@test("1.2 qc_compute service is active")
def t_qc_service():
    r = os.popen("systemctl is-active qc_compute").read().strip()
    if r != "active":
        return "qc_compute is %s" % r
    return True
...
@test("1.4 qc_compute /health returns ok")
def t_qc_health():
    r = qc_get("/health")
    d = r.json()
    if not d.get("ok"):
...
@test("5.1 Create test signal in PB signals collection")
def t_create_signal():
    ...
    r = pb_post("/api/collections/signals/records", signal_data)
...
def t_signal_router():
    ...
    signals = router.fetch_pending_signals()
...
def t_cleanup_signal():
    global _test_signal_id
    if _test_signal_id:
        pb_delete("/api/collections/signals/records/%s" % _test_signal_id)
...
@test("7.2 /ibkr/dashboard serves HTML")
def t_ibkr_dashboard():
    r = qc_get("/ibkr/dashboard")
    if r.status_code != 200:
        return "status %d" % r.status_code
    if "Paper Trading Dashboard" not in r.text:
        return "dashboard content missing"
    return True
...
def t_bar_align():
    from qc_compute.market.bar_aggregator import get_bar_interval_start
    from datetime import datetime, timezone, timedelta
    ET = timezone(timedelta(hours=-4))
...
def main():
    ...
    print("  Gateway: %s  |  PB: %s  |  QC: %s" % (GW_URL, PB_URL, QC_URL))
    ...
    if gw_ok:
        ...
        print("  Run: source /opt/qc_compute/.env && PYTHONPATH=/opt/qc_compute/src python3 /opt/qc_compute/deploy/ibkr_login.py")
    print()
