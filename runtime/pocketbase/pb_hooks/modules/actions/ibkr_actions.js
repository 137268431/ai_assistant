...
function ibkrDataQualityParseBoolean(value, fallback) {
    if (value === undefined || value === null || value === "") return fallback
    const normalized = String(value || "").trim().toLowerCase()
    if (["true", "1", "yes", "y"].indexOf(normalized) !== -1) return true
    if (["false", "0", "no", "n"].indexOf(normalized) !== -1) return false
...
function ibkrDataQualitySerializeIntegrityRecord(record) {
    return {
        id: record.id || "",
        environment: record.get("environment") || "",
        market_date: record.get("market_date") || "",
...
function ibkrDataQualitySerializeTruthRecord(record) {
    return {
        id: record.id || "",
        environment: record.get("environment") || "",
        market_date: record.get("market_date") || "",
...
function ibkrDataQualityLatestByKey(records, serialize, keyBuilder) {
    const items = []
    const seen = {}
...
function ibkrDataQualityLoadEffectiveWatchlistSymbols(environment) {
    const runtimeEnvironment = String(environment || "live").trim().toLowerCase() || "live"
    const rows = $app.findRecordsByFilter(
        "watchlist",
...
function ibkrDataQualityLoadIntegrityItems(environment, opts) {
    const options = opts || {}
    ...
    return globalThis.ibkrDataQualityLatestByKey(
        rows,
        globalThis.ibkrDataQualitySerializeIntegrityRecord,
        (item) => `${String(item.symbol || "").trim().toUpperCase()}::${String(item.interval || "5m").trim()}`
    )
}

function ibkrDataQualityLoadTruthItems(environment, opts) {
    const options = opts || {}
    const filterParts = ['environment = {:env}']
    const filterParams = { env: environment }
    if (options.market_date) {
    ...
    return globalThis.ibkrDataQualityLatestByKey(
        rows,
        globalThis.ibkrDataQualitySerializeTruthRecord,
        (item) => `${String(item.symbol || "").trim().toUpperCase()}::${String(item.interval || "5m").trim()}`
    )
}

function ibkrDataQualityResolveProof(item) {
...
function ibkrDataQualityMergeItems(integrityItems, truthItems) {
    const merged = {}
    ...
    for (let i = 0; i < (truthItems || []).length; i++) {
        ...
        base.proof = globalThis.ibkrDataQualityResolveProof(base)
        merged[key] = base
    }
    const items = Object.keys(merged).map((key) => {
        const item = merged[key]
        item.proof = globalThis.ibkrDataQualityResolveProof(item)
        return item
    })
...
function ibkrDataQualityBuildSummary(environment, marketDate, scanScope, mergedItems, expectedSymbols) {
    ...
    for (let i = 0; i < expected.length; i++) {
        expectedSet[String(expected[i] || "").trim().toUpperCase()] = true
    }
...
function ibkrDataQualityBuildTruthSummary(environment, marketDate, truthItems, expectedSymbols) {
...
globalThis.ibkrDataQualityParseBoolean = ibkrDataQualityParseBoolean
globalThis.ibkrDataQualitySerializeIntegrityRecord = ibkrDataQualitySerializeIntegrityRecord
globalThis.ibkrDataQualitySerializeTruthRecord = ibkrDataQualitySerializeTruthRecord
globalThis.ibkrDataQualityLatestByKey = ibkrDataQualityLatestByKey
globalThis.ibkrDataQualityLoadEffectiveWatchlistSymbols = ibkrDataQualityLoadEffectiveWatchlistSymbols
globalThis.ibkrDataQualityLoadIntegrityItems = ibkrDataQualityLoadIntegrityItems
globalThis.ibkrDataQualityLoadTruthItems = ibkrDataQualityLoadTruthItems
globalThis.ibkrDataQualityResolveProof = ibkrDataQualityResolveProof
globalThis.ibkrDataQualityMergeItems = ibkrDataQualityMergeItems
globalThis.ibkrDataQualityBuildSummary = ibkrDataQualityBuildSummary
globalThis.ibkrDataQualityBuildTruthSummary = ibkrDataQualityBuildTruthSummary
...
routerAdd("POST", "/api/custom/ibkr/data_quality/truth_upsert", (c) => {
    const reqInfo = c.requestInfo()
    const d = reqInfo.body || reqInfo.data || {}
    const items = Array.isArray(d.items) ? d.items : []
    const { getRuntimeEnvironmentFromData, LIVE_ENVIRONMENT } = require(`${__hooks}/lib/environment.js`)
    ...
    if (!items.length) {
        return c.json(400, { ok: false, error: "Empty truth audit items array" })
    }
    ...
    for (let i = 0; i < items.length; i++) {
        const environment = getRuntimeEnvironmentFromData(items[i], defaultEnvironment)
        const prepared = actionHelpers.buildBarTruthAuditData(items[i] || {}, environment)
        if (!prepared.ok) {
            errors++
            continue
        }
        try {
            const result = actionHelpers.upsertRecord("ibkr_bar_truth_audit", prepared.filter, prepared.params, prepared.data)
            if (result.action === "created") {
                created++
            } else if (result.action === "updated") {
                updated++
            ...
            errors++
            console.error(`[IBKRActions] truth audit upsert error: ${prepared.symbol}/${prepared.market_date}: ${err.message}`)
        }
    }
...
routerAdd("GET", "/api/custom/ibkr/data_quality/summary", (c) => {
    ...
    try {
        const integrityItems = dataQuality.loadIntegrityItems(environment, {
            market_date: marketDate,
            scan_scope: scanScope,
        })
        const truthItems = dataQuality.loadTruthItems(environment, {
            market_date: marketDate,
        })
        const mergedItems = dataQuality.mergeItems(integrityItems, truthItems)
        const expectedSymbols = (!scanScope || scanScope === "watchlist")
            ? dataQuality.loadEffectiveWatchlistSymbols(environment)
            : mergedItems.map((item) => String(item.symbol || "").trim().toUpperCase()).filter(Boolean)
        const summary = dataQuality.buildSummary(environment, marketDate, scanScope, mergedItems, expectedSymbols)
...
routerAdd("GET", "/api/custom/ibkr/data_quality/truth_summary", (c) => {
    const { getRuntimeEnvironmentFromRequest, LIVE_ENVIRONMENT } = require(`${__hooks}/lib/environment.js`)
    const dataQuality = require(`${__hooks}/lib/trading/ibkr_data_quality.js`)
    const environment = getRuntimeEnvironmentFromRequest(c, LIVE_ENVIRONMENT)
    const query = c.request.url.query()
    const marketDate = String(query.get("market_date") || "").trim()
    try {
        const truthItems = dataQuality.loadTruthItems(environment, { market_date: marketDate })
        const expectedSymbols = dataQuality.loadEffectiveWatchlistSymbols(environment)
        const summary = dataQuality.buildTruthSummary(environment, marketDate, truthItems, expectedSymbols)
        return c.json(200, { ok: true, summary })
    } catch (err) {
        return c.json(500, { ok: false, error: err.message || String(err) })
    }
...
routerAdd("GET", "/api/custom/ibkr/data_quality/list", (c) => {
    try {
        ...
        const needsRepair = dataQuality.parseBoolean(query.get("needs_repair"), null)
        ...
        let items = dataQuality.mergeItems(integrityItems, truthItems)
        if (status) {
            items = items.filter((item) => String(item.status || "").trim().toLowerCase() === status)
        }
        if (needsRepair !== null) {
            items = items.filter((item) => Boolean(item.needs_repair) === needsRepair)
        }
...
routerAdd("GET", "/api/custom/ibkr/data_quality/truth_list", (c) => {
    try {
        const { getRuntimeEnvironmentFromRequest, LIVE_ENVIRONMENT } = require(`${__hooks}/lib/environment.js`)
        const dataQuality = require(`${__hooks}/lib/trading/ibkr_data_quality.js`)
        const environment = getRuntimeEnvironmentFromRequest(c, LIVE_ENVIRONMENT)
        ...
        const page = Math.max(1, Number(query.get("page") || 1) || 1)
        const perPage = Math.max(1, Math.min(200, Number(query.get("per_page") || 50) || 50))
        let items = dataQuality.loadTruthItems(environment, {
            market_date: marketDate,
            symbol: symbol,
        })
        if (status) {
...
function ibkrDataQualityPersistRows(environment, route, items, timeoutSeconds) {
...
function ibkrDataQualityRunProxyAction(environment, route, upstream, timeoutSeconds, persistRoute, body) {
    const resp = $http.send({
        url: upstream,
        method: "POST",
        timeout: timeoutSeconds,
    ...
    if (Array.isArray(payload.rows) && payload.rows.length && persistRoute) {
        try {
            payload.persistence = globalThis.ibkrDataQualityPersistRows(environment, persistRoute, payload.rows, 20)
        } catch (persistErr) {
...
globalThis.ibkrDataQualityPersistRows = ibkrDataQualityPersistRows
globalThis.ibkrDataQualityRunProxyAction = ibkrDataQualityRunProxyAction
...
routerAdd("POST", "/api/custom/ibkr/data_quality/rescan", (c) => {
    try {
        ...
        const result = dataQuality.runProxyAction(
            ...
            upstream,
            180,
            "http://127.0.0.1:8090/api/custom/ibkr/data_quality/upsert",
            d,
        )
...
routerAdd("POST", "/api/custom/ibkr/data_quality/truth_audit", (c) => {
    try {
        const reqInfo = c.requestInfo()
        const d = reqInfo.body || reqInfo.data || {}
        const { getRuntimeEnvironmentFromData, getIbkrComputePublicUrl, LIVE_ENVIRONMENT } = require(`${__hooks}/lib/environment.js`)
        ...
        const result = dataQuality.runProxyAction(
            ...
            upstream,
            600,
            "http://127.0.0.1:8090/api/custom/ibkr/data_quality/truth_upsert",
            d,
        )
        return c.json(result.statusCode, result.payload)
    } catch (err) {
...
routerAdd("POST", "/api/custom/ibkr/2fa/takeover", (c) => {
    ...
    const { proxyIbkrApiJson } = require(`${__hooks}/lib/system/api_proxy.js`)
    const { getRuntimeEnvironmentFromData, LIVE_ENVIRONMENT } = require(`${__hooks}/lib/environment.js`)
    return proxyIbkrApiJson(c, "/api/custom/ibkr/2fa/takeover", {
        method: "POST",
        environment: getRuntimeEnvironmentFromData(d, LIVE_ENVIRONMENT),
        body: d,
        timeout: 20,
...
routerAdd("POST", "/api/custom/ibkr/2fa/probe", (c) => {
    ...
    const { proxyIbkrApiJson } = require(`${__hooks}/lib/system/api_proxy.js`)
    const { getRuntimeEnvironmentFromData, LIVE_ENVIRONMENT } = require(`${__hooks}/lib/environment.js`)
    return proxyIbkrApiJson(c, "/api/custom/ibkr/2fa/probe", {
        method: "POST",
        environment: getRuntimeEnvironmentFromData(d, LIVE_ENVIRONMENT),
        body: d,
        timeout: 20,
...
routerAdd("POST", "/api/custom/ibkr/2fa/panic-reset", (c) => {
    ...
    const { proxyIbkrApiJson } = require(`${__hooks}/lib/system/api_proxy.js`)
    const { getRuntimeEnvironmentFromData, LIVE_ENVIRONMENT } = require(`${__hooks}/lib/environment.js`)
    return proxyIbkrApiJson(c, "/api/custom/ibkr/2fa/panic-reset", {
        method: "POST",
        environment: getRuntimeEnvironmentFromData(d, LIVE_ENVIRONMENT),
        body: d,
        timeout: 60,
