...
CRON_DEFINITIONS: list[dict[str, Any]] = [
    ...
    _definition(
        ...
        "按 09:20 ET 触发信号窗口 daily scan，刷新当天 candidate / active 目标池。",
    ...
    _definition(
        ...
        133.2,
        "29,30,40,50 9 * * 1-5",
        "工作日 ET 09:29/09:30/09:40/09:50",
        "09:20 主池后，09:29 ET 先做开盘预采集，并在 09:30-10:30 ET 每 10 分钟执行增量扩池；只增加新可操作标的并按需提醒。",
        "由 ibkr-api 触发 compute topup scan 并负责新增标的提醒；使用 America/New_York 时区匹配。",
        beijing_cycle_label="北京时间 工作日 21:29 预采集后 21:30-22:30（美东夏令时）/ 22:29 预采集后 22:30-23:30（美东冬令时）每 10 分钟",
        et_cycle_label="美东时间 工作日 09:29 预采集 + 09:30-10:30 每 10 分钟",
        window_label="09:29 ET 开盘预采集 + 09:30-10:30 ET 信号窗口入池补充",
        hook_file="ibkr_system_monitor.pb.js",
        runner_kind="native_api_http",
        mode_scope="market_data",
        cron_timezone="America/New_York",
