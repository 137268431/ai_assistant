...
CRON_DEFINITIONS: list[dict[str, Any]] = [
    ...
    _definition(
        "ibkr_cache_prewarm",
        "pb_cron_ibkr_cache_prewarm_enabled",
        "页面快照预热 Cron",
        "页面快照预热",
        132.35,
        "*/1 4-20 * * 1-5",
        "工作日 America/New_York 04:00-20:59 每 1 分钟",
        "预热首页、system summary lite 与 universe 重读页面的 PocketBase 快照，降低单用户冷数据首屏耗时。",
        "只调用 ibkr-api 读页面 builder 并写入 ibkr_cache_snapshots；交易下单与运行时真值仍以实时接口为准。",
        beijing_cycle_label="北京时间 工作日 16:00-08:59（美东夏令时）/ 17:00-09:59（美东冬令时）每 1 分钟",
        et_cycle_label="美东时间 工作日 04:00-20:59 每 1 分钟",
        window_label="盘前 / 盘中 / 盘后页面快照预热",
        runner_kind="native_api_http",
        mode_scope="market_data",
        cron_timezone="America/New_York",
        family="system_monitor",
...
NATIVE_API_HTTP_JOB_ENDPOINTS: dict[str, tuple[str, str]] = {
    ...
    "ibkr_cache_prewarm": ("POST", "/api/custom/system/jobs/cache_prewarm"),
