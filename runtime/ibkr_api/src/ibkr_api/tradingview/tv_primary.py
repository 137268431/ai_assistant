...
def _route_entry(
    ...
    *,
    ...
    target = _load_target(pb, symbol=symbol, date=_market_date(payload), environment=environment, escape_filter=escape_filter)
    ...
    if window == "quality":
        rank = _int(target_extra.get("activity_rank"), 999999)
        max_rank = _config_int(config_value, "tv_quality_window_max_rank", 5, environment)
