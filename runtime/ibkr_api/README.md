...
- Native scheduler-triggered system jobs now live under `runtime/ibkr_api/src/ibkr_api/system/jobs/` and `runtime/ibkr_api/src/ibkr_api/system/routes.py`, including `order_expiry`, auth/session guards, data-gap guard, weekly 2FA reminders, and open/close reminder dispatch.
...
- Current migration boundary is still mixed:
  - Native in `ibkr-api`: signal/order/reverse routes, webhook pages, system `healthz|summaryz|monitorz|cronz|schedulerz`, runtime `statusz|healthz|runtime/config|startup/*|2fa/status`, control actions `emergency-stop|recover|reauth`, all current 2FA control routes, state `signals|orders`, plus `health-report` and `notify`.
  - Still delegated back to PocketBase through `compat/routes.py` fallback: large parts of `pb_hooks/modules/actions/ibkr_actions.js`, including bar/indicator ingest, today-targets/watchlist/targets, data-quality/screener/quotes/history rebuild, runtime order execution and account views, and `orders/cancel_sync`.
