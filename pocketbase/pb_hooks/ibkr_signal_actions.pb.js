  ...
  try {
    ...
    function findLatestIndicator(signalRecord) {
      ...
      function query(filter, params) {
        const rows = $app.findRecordsByFilter("ibkr_indicators", filter, "-bar_time_ms", 1, 0, params)
    ...
    const ibkr_signals = records.map((r) => {
    ...
    const matchedIndicators = ibkr_signals.filter((s) => !!s.latest_indicator).length
    console.log(`[SignalsPending] 返回 ibkr_signals 数组长度: ${ibkr_signals.length}`);
    console.log(`[SignalsPending] 已关联指标快照: ${matchedIndicators}/${ibkr_signals.length}`);
...
routerAdd("POST", "/api/custom/ibkr/signals/ack", (c) => {
  ...
  const environment = getSignalActionsDataEnvironment(data);
