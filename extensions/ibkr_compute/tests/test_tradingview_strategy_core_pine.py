...
class TradingViewStrategyCorePineTest(unittest.TestCase):
    ...
    def test_probe_entries_support_predictive_and_confirmed_2b_orders(self):
        ...
        self.assertIn('topVolumeClimaxStructureReady = strongTopProbeZone and topRetestBand', source)
        ...
        self.assertIn('topConfirmedShort = topSweepFailed or upperExhaustionConfirmedShort or topWeakNewHighConfirmed or topDistributionDropConfirmed or topVolumeClimaxFailedBreakConfirmed', source)
        self.assertIn('topConfirmVolumeOk = topHighVolumeFailedBreak or not attackUpCooling or upperExhaustionConfirmedShort or topWeakNewHighConfirmed or topDistributionDropConfirmed or topVolumeClimaxFailedBreakConfirmed', source)
        self.assertIn('topConfirmStop = topConfirmedShort ? (topVolumeClimaxFailedBreakConfirmed ? math.max(nz(lastTopVolumeClimaxHigh, high), high) + invalidationBuffer', source)
        ...
        self.assertIn('topConfirmContext = topConfirmedShort and (topCounterTrendAllowed or upperExhaustionConfirmedShort or topWeakNewHighConfirmed or topDistributionDropConfirmed or topVolumeClimaxFailedBreakConfirmed) and topSignalGapOk and topConfirmVolumeOk', source)
    ...
    def test_display_explains_event_flow_and_four_plan_lines(self):
        ...
        self.assertIn('②冷却中', display)
        self.assertIn('reasonCn(blockedVolumeReason)', display)
        ...
        self.assertIn('冲顶观察 空', display)
        self.assertIn('放量新高', display)
        self.assertIn('等红K收回前高', display)
        self.assertIn('volumeClimaxTopConfirmEvent', display)
        self.assertIn('③冷却解除', display)
        self.assertIn('冲顶失败确认 空', display)
        self.assertIn('红K收回前高', display)
