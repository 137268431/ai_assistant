...
class TradingViewStrategyCorePineTest(unittest.TestCase):
    ...
    def test_entry_logic_requires_window_and_two_of_three_direction_components(self):
        ...
        self.assertIn("bool setupLongTrendUpper = setupLongTrendUpperBase or setupLongTrendUpperPullback", source)
        ...
        self.assertIn("int scoreLongTrendUpper = trendLongPullbackContinuationQualified ? 100 : math.min(100,", source)
        ...
        self.assertIn("int scoreShortTrendLower = trendShortPullbackContinuationQualified ? 100 : math.min(100, (shortTrendContextReady ? 25 : 0) + (lowerBearTrendTriggerSeen ? 25 : 0)", source)
        ...
        self.assertIn('string candidateDirectionDecisionText = trendLongPullbackContinuationQualified and candidateSetup == "trend_sdUpper" ? "\\n方向裁决: DTP强多，趋势回踩确认，区间突破/快拒替代等待多背离"', display)
    ...
    def test_structural_anchor_entry_limit_payload_and_pending_ttl(self):
        ...
        self.assertIn('bool longTrendReclaimEntry = longSetup == "trend_sdUpper" and setupLongTrendUpperPullback', source)
        self.assertIn('bool shortTrendReclaimEntry = shortSetup == "trend_sdLower" and setupShortTrendLowerContinuation and trendShortPullbackContinuationQualified', source)
        ...
        self.assertIn('jsonStr("entry_price_plan", trendReclaimEntry ? "trend_reclaim_bounded_limit" : structuralAnchorMode ? "structural_anchor_limit" : "tv_direct_bounded_limit")', source)
        ...
        self.assertIn("float submittedLimitPrice = longTrendReclaimEntry or not structuralAnchorMode ? calcSubmittedEntryLimit(entryPrice, 1) : entryPrice", source)
        self.assertIn("float submittedLimitPrice = shortTrendReclaimEntry or not structuralAnchorMode ? calcSubmittedEntryLimit(entryPrice, -1) : entryPrice", source)
    ...
    def test_display_pending_entry_lifecycle_is_visual_only(self):
        ...
        self.assertIn("bool historicalPlanSnapshotNow = showHistoricalPlanLines and (displayEnterLong or displayEnterShort) and not na(displaySignalEntryPlan)", display)
        ...
        self.assertIn('string pendingDisplayStatus = displayShadowExitNow ? "Shadow退出" : displayShadowOpenPosition ? "Shadow持仓中" : displayPendingEntryTouchedNow ? "Core模拟触及" : displayPendingCancelNow ? "撤销挂单" : pendingDisplayWaiting ? "Core挂单中" : ""', display)
