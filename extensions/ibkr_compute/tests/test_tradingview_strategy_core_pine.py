...
class TradingViewStrategyCorePineTest(unittest.TestCase):
    ...
    def test_structural_anchor_entry_limit_payload_and_pending_ttl(self):
        ...
        self.assertIn("bool pendingSameSignalStillValid = confirmed and activePendingEntry and strategy.position_size == 0", source)
        self.assertIn("longSignalCandidate and longSetup == activeSetup and not blockLong", source)
        self.assertIn("shortSignalCandidate and shortSetup == activeSetup and not blockShort", source)
        self.assertIn("bool pendingEntryRenewNow = pendingEntryExpired and pendingSameSignalStillValid", source)
