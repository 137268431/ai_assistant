...
class TradingViewStrategyCorePineTest(unittest.TestCase):
    ...
    def test_probe_entries_support_predictive_and_confirmed_2b_orders(self):
        ...
        for text in (source, display):
            ...
            self.assertIn('minMiddleSwingAtr = input.float(0.90, "Middle swing for second test (x ATR)"', text)
        ...
        self.assertIn('topAnchorReady = topAnchorAgeBars >= probeMinAnchorAgeBars and topPullbackAtr >= math.max(probeMinPullbackAtr, minMiddleSwingAtr)', source)
        self.assertIn('bottomAnchorReady = bottomAnchorAgeBars >= probeMinAnchorAgeBars and bottomPullbackAtr >= math.max(probeMinPullbackAtr, minMiddleSwingAtr)', source)
    ...
    def test_display_explains_event_flow_and_four_plan_lines(self):
        ...
        self.assertIn('"\\n同向距离 " + fmt(selectedSameSideSignalDistanceAtr)', display)
