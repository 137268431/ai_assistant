...
class TradingViewStrategyCorePineTest(unittest.TestCase):
    ...
    def test_structure_first_tp_sl_uses_atr_only_as_buffer_and_filter(self):
        ...
        self.assertIn("float longTrendStructureTargetRaw = setupUsesRunner(longSetup) and not na(longTrendSafetyTargetRaw) ? longTrendSafetyTargetRaw : longTrendNearestTargetRaw", source)
        self.assertIn("float shortTrendStructureTargetRaw = setupUsesRunner(shortSetup) and not na(shortTrendSafetyTargetRaw) ? shortTrendSafetyTargetRaw : shortTrendNearestTargetRaw", source)
        self.assertIn("float longTargetCandidate = longStructureTargetRaw", source)
        self.assertIn("float shortTargetCandidate = shortStructureTargetRaw", source)
    ...
    def test_signal_diagnostics_draw_filtered_candidates_without_alerts(self):
        ...
        self.assertIn("if flowLabelsVisible and showSignalDiagnostics and blockedFlowVisible and not pendingDisplayWaiting and blockedNow and blockedVisualKey != lastBlockVisualKey", source)
    ...
    def test_display_pending_entry_lifecycle_is_visual_only(self):
        ...
        self.assertIn("bool flowPlanLinesShouldExtend = showHistoricalPlanLines and (pendingDisplayWaiting or displayPendingEntryTouchedNow or displayPendingCancelNow)", display)
        ...
        self.assertIn('string pendingDisplayStatus = displayPendingEntryTouchedNow ? "Core模拟触及" : displayPendingCancelNow ? "撤销挂单" : pendingDisplayWaiting ? "Core挂单中" : ""', display)
        self.assertIn('string statusText = displayEnterLong or displayEnterShort ? "Core挂单中"', display)
        self.assertIn('string statusComponents = pendingDisplayStatus != "" ? displayPendingCancelNow ? "撤销挂单: " + cancelReasonDisplayName(displayPendingCancelReason) : "成交确认看 Core"', display)
        self.assertIn('string planLineLegend = showCurrentPlanLines or showHistoricalPlanLines ? " / 黄Plan 红SL 绿TP 蓝Runner" : ""', display)
