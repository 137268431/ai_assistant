...
class TradingViewStrategyCorePineTest(unittest.TestCase):
    ...
    def test_window_activation_pre_alert_is_non_directional(self):
        ...
        self.assertIn("var bool observationNoticeSentToday = false", source)
        self.assertIn("if isNewTradingDay\n    observationNoticeSentToday := false", source)
        self.assertIn("bool lowerObservationNotice = canTrade and flatForEntry and lowerActivationEvent and not observationNoticeSentToday", source)
        self.assertIn("bool upperObservationNotice = canTrade and flatForEntry and upperActivationEvent and not observationNoticeSentToday and not lowerObservationNotice", source)
        self.assertIn("if lowerObservationNotice or upperObservationNotice\n    observationNoticeSentToday := true", source)
    ...
    def test_activation_labels_mark_observe_pool_not_entry(self):
        ...
        self.assertIn("if showFlowLabels and lowerObservationNotice", source)
        self.assertIn("if showFlowLabels and upperObservationNotice", source)
