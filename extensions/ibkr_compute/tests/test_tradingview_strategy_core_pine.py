...
class TradingViewStrategyCorePineTest(unittest.TestCase):
    ...
    def test_probe_entries_support_predictive_and_confirmed_2b_orders(self):
        ...
        self.assertIn('topConfirmEntry = topConfirmedShort ? (topCloseLimitShort ? close + syminfo.mintick', source)
    ...
    def test_runner_keeps_far_safety_tp_and_updates_stop(self):
        ...
        self.assertIn('structureSafetyTarget(float entryPrice, float rTarget, int direction, float structureLevel, float bufferValue) =>', source)
        self.assertIn('floatingStructureStop(int direction, float currentStop, float pivotLowValue, float pivotHighValue, float currentClose, float bufferValue) =>', source)
        self.assertIn('looserStop(float rawStop, float structureStop, int direction) =>', source)
        self.assertIn('selectedTargetByR = selectedDirection != 0 ? calcTargetAtR(selectedEntry, selectedStop, selectedDirection, runnerSafetyR)', source)
        self.assertIn('topProbeTargetBase = na(lowSinceStructHigh[1]) ? lowSinceStructHigh : lowSinceStructHigh[1]', source)
        self.assertIn('bottomProbeTargetBase = na(highSinceStructLow[1]) ? highSinceStructLow : highSinceStructLow[1]', source)
        self.assertIn('selectedStructureTargetBase = selectedLong ? (selectedTrendSupport ? todayStructHigh : bottomProbeTargetBase) : selectedShort ? (selectedTrendResistance ? todayStructLow : topProbeTargetBase) : na', source)
        self.assertIn('selectedTarget = selectedDirection != 0 ? structureSafetyTarget(selectedEntry, selectedTargetByR, selectedDirection, selectedStructureTargetBase, invalidationBuffer)', source)
        self.assertIn('riskReason := "runner_activation"', source)
        self.assertIn('rawTrailStop = activeDirection == 1 ? activeRunnerExtreme - atr * runnerTrailAtrMult : activeRunnerExtreme + atr * runnerTrailAtrMult', source)
        self.assertIn('structureTrailStop = floatingStructureStop(activeDirection, activeStop, lastPivotLow, lastPivotHigh, close, invalidationBuffer)', source)
        self.assertIn('riskReason := riskReason == "" ? "runner_trail_stop" : riskReason', source)
        self.assertIn('structureCompressStop = floatingStructureStop(activeDirection, activeStop, lastPivotLow, lastPivotHigh, close, invalidationBuffer)', source)
        self.assertIn('rawCompressedStop = activeEntry - activeInitialRisk * noFollowThroughCompressedStopR * activeDirection', source)
    ...
    def test_display_explains_event_flow_and_four_plan_lines(self):
        ...
        self.assertIn('structureSafetyTarget(float entryPrice, float rTarget, int direction, float structureLevel, float bufferValue) =>', display)
        ...
        self.assertIn('selectedTargetByR = selectedDirection != 0 ? calcTargetAtR(selectedEntry, selectedStop, selectedDirection, runnerSafetyR)', display)
        self.assertIn('topProbeTargetBase = na(lowSinceStructHigh[1]) ? lowSinceStructHigh : lowSinceStructHigh[1]', display)
        self.assertIn('bottomProbeTargetBase = na(highSinceStructLow[1]) ? highSinceStructLow : highSinceStructLow[1]', display)
        self.assertIn('selectedTarget = selectedDirection != 0 ? structureSafetyTarget(selectedEntry, selectedTargetByR, selectedDirection, selectedStructureTargetBase, invalidationBuffer)', display)
