...
class TradingViewStrategyCorePineTest(unittest.TestCase):
    ...
    def test_probe_entries_support_predictive_and_confirmed_2b_orders(self):
        ...
        self.assertIn('topConfirmEntry = topConfirmedShort ? (topCloseLimitShort ? topConfirmEntryRaw : scalpLimitEntry(-1, close, topBasis, topConfirmEntryRaw, atr, scalpLimitOffsetAtr)) : na', source)
    ...
    def test_quick_ema_scalp_uses_ema_target_rsi_filter_and_small_stop(self):
        ...
        self.assertIn('trendLongConfirmEntry = strongTrendSupport ? scalpLimitEntry(1, close, trendSupportBasis', source)
        self.assertIn('trendShortConfirmEntry = strongTrendResistance ? scalpLimitEntry(-1, close, trendResistanceBasis', source)
        ...
        self.assertIn('topConfirmEntry = topConfirmedShort ? (topCloseLimitShort ? topConfirmEntryRaw : scalpLimitEntry(-1, close, topBasis', source)
        self.assertIn('bottomFailureRetestEntry = nz(lastBottomFailureWatchClose, close) - retestCloseEntryOffset', source)
        self.assertIn('bottomConfirmEntry = bottomConfirmedLong ? (bottomCloseLimitLong ? bottomConfirmEntryRaw : scalpLimitEntry(1, close, bottomBasis', source)
    ...
    def test_display_explains_event_flow_and_four_plan_lines(self):
        ...
        self.assertIn('showEntryWindowStatus = input.bool(true, "Show entry window status"', display)
        ...
        self.assertIn('entryWindowBgColor(bool windowEnabled, bool windowOpen) =>', display)
        self.assertIn('entryWindowTextColor(bool windowEnabled, bool windowOpen) =>', display)
        ...
        for line in display.splitlines():
            ...
            if "table.cell(" in line:
                self.assertTrue("text_color=tableTextColor(" in line or "text_color=entryWindowTextColor(" in line)
                self.assertTrue("bgcolor=tableBgColor(" in line or "bgcolor=entryWindowBgColor(" in line)
        ...
        self.assertIn('var table entryWindowTable = table.new(position.bottom_right, 1, 2)', display)
        self.assertIn('"交易窗口 OPEN"', display)
        self.assertIn('"交易窗口 CLOSED"', display)
        self.assertIn('"交易窗口 FILTER OFF"', display)
        self.assertIn('(inEntryWindow ? "可挂单 " : "仅观察 ") + entrySession', display)
        self.assertIn('ENTRY_WINDOW_MODEL + " / " + (useIntradayEntryWindow ? entrySession : "窗口过滤关闭")', display)
        ...
        self.assertIn('eventMarker(distributionDirection, distributionAllowed ? "candidate" : "blocked"', display)
        self.assertIn('eventMarker(-1, topConfirmAllowed ? "candidate" : "blocked"', display)
        ...
        self.assertIn('"\\n强结构条件: " + selectedStructureGateFlags', display)
        ...
        self.assertIn('裸K反转已触发候选，但强结构门槛未通过，所以不挂单', display)
        ...
        self.assertIn('"Entry / SL / 0.6R / EMA20TP"', display)
