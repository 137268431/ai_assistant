...
class TradingViewStrategyCorePineTest(unittest.TestCase):
    ...
    def test_probe_entries_support_predictive_and_confirmed_2b_orders(self):
        ...
        for text in (source, display):
            ...
            self.assertIn('"no_middle_swing"', text)
        ...
        self.assertIn('topMiddleSwingReady = not na(lowSinceStructHighBar) and topPullbackAtr >= math.max(probeMinPullbackAtr, minMiddleSwingAtr)', source)
        self.assertIn('topRetestFastEnough = topMiddleSwingReady and bar_index > lowSinceStructHighBar and bar_index - lowSinceStructHighBar <= maxRetestAfterMiddleSwingBars', source)
        self.assertIn('topAnchorReady = topAnchorAgeBars >= probeMinAnchorAgeBars and topRetestFastEnough', source)
        self.assertIn('bottomMiddleSwingReady = not na(highSinceStructLowBar) and bottomPullbackAtr >= math.max(probeMinPullbackAtr, minMiddleSwingAtr)', source)
        self.assertIn('bottomRetestFastEnough = bottomMiddleSwingReady and bar_index > highSinceStructLowBar and bar_index - highSinceStructLowBar <= maxRetestAfterMiddleSwingBars', source)
    ...
    def test_runner_keeps_far_safety_tp_and_updates_stop(self):
        ...
        self.assertIn('(direction == 1 ? "L" : "S") + " E" + fmt(entryPrice) + " Q"', source)
        self.assertIn('" SL" + fmt(stopPrice) + " TP" + fmt(targetPrice) + " R" + fmt(runnerPrice)', source)
        self.assertIn('reason == "runner_stop" ? "RUN"', source)
        self.assertIn('exitReasonDisplayNameCore(reason) + " " + fmtSignedMoneyCore(pnlValue) + " " + fmtSignedRCore(pnlR) + " X" + fmt(exitPrice)', source)
    ...
    def test_display_explains_event_flow_and_four_plan_lines(self):
        ...
        self.assertIn('"\\n中间波段 " + fmt(selectedProbePullbackAtr)', display)
