...
const configData = [
  ...
  cfg('entry_limit_mode', 'marketable_limit_dynamic', 'marketable_limit_dynamic', '入场价格模式', '信号与反转', 222.55, 'marketable_limit_dynamic=close 基础上按 5m ATR 与 bps clamp 生成可成交 limit；marketable_limit_bps=旧版固定 bp 偏移'),
  cfg('entry_limit_atr_mult', '0.30', '0.30', '动态入场 ATR 倍数', '信号与反转', 222.56, '动态入场 offset 的 ATR 倍数；默认使用 5m ATR*0.30'),
  cfg('entry_limit_floor_bps', '15', '15', '动态入场最小bp', '信号与反转', 222.57, '动态入场 offset 下限，避免 ATR 太小时挂单不易成交'),
  cfg('entry_limit_cap_bps', '30', '30', '动态入场最大bp', '信号与反转', 222.58, '动态入场 offset 上限，避免为追成交付出过高滑点'),
  cfg('marketable_limit_bps', '10', '10', 'Marketable Limit 偏移bp', '信号与反转', 222.6, '旧版 fixed bps 入场模式使用；动态入场模式默认使用 ATR clamp 参数'),
  ...
  cfg('ibkr_watchlist_idle_topup_materialize_5m', 'FALSE', 'FALSE', '回补后物化 5m', '行情链路', 707.1, '实验开关：回补后强制物化 5m bars；默认关闭，避免重复写入链路放大'),
