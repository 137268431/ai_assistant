...
const configData = [
  ...
  cfg('pb_cron_ibkr_scan_runtime_enabled', 'TRUE', 'TRUE', '信号窗口预筛', 'Scheduler 调度(兼容 key)', 133, '按 08:20-09:20 ET 每 5 分钟触发信号窗口 daily scan，覆盖刷新当天 candidate / active 目标池。Cron: 20,25,30,35,40,45,50,55 8 * * 1-5 与 0,5,10,15,20 9 * * 1-5；时区: America/New_York。受 Scheduler 总开关、Compute 开关和本开关共同控制。'),
  cfg('pb_cron_ibkr_fundamentals_refresh_enabled', 'TRUE', 'TRUE', 'Fundamentals 刷新', 'Scheduler 调度(兼容 key)', 133.05, '盘前小批量刷新 trade watchlist 的股票基础数据。Cron: 5 7 * * 1-5；时区: America/New_York；周期: 工作日 ET 07:05。'),
  ...
  cfg('intraday_symbol_daily_entry_limit', '2', '2', '标的每日入场上限', '信号与反转', 222.6967, 'controlled 策略下同一标的每天最多实际入场次数；0 表示关闭'),
  ...
  cfg('tv_max_active_targets', '10', '10', 'TV 活跃标的上限', '行情链路', 703, 'TradingView pre_alert 入池后，系统按 activity_score / quality_score 排序，每日最多激活多少个标的'),
  cfg('tv_max_same_direction_targets', '7', '7', 'TV 同方向上限', '行情链路', 7031, 'TV 活跃标的排行中同一 direction_bias 最多保留多少个 active；0 表示不限制方向'),
  cfg('tv_entry_requires_active_target', 'TRUE', 'TRUE', 'TV 入场要求 Active 标的', '行情链路', 705, 'TRUE 时 entry webhook 必须命中当日 active ibkr_targets；避免未入池标的直接下单'),
  ...
  cfg('tv_entry_primary_start', '09:40', '09:40', 'TV 主入场开始', '行情链路', 7071, '美东时间，默认 09:40；避免开盘前 10 分钟噪声'),
  cfg('tv_entry_primary_end', '11:30', '11:30', 'TV 主入场结束', '行情链路', 7072, '美东时间，09:40-11:30 为主入场窗口'),
  cfg('tv_entry_quality_end', '14:30', '14:30', 'TV 高质量入场截止', '行情链路', 7073, '美东时间，11:30-14:30 只允许排名和质量更高的 late entry；14:30 后不新开仓'),
