...
const configData = [
  ...
  cfg('pb_cron_ibkr_compute_runtime_enabled', 'TRUE', 'TRUE', 'Compute + 状态摘要', 'Scheduler 调度(兼容 key)', 132, '触发 compute 调度，并串行执行系统健康检查与状态摘要通知。Cron: */5 4-20 * * 1-5；周期: 工作日 UTC 04:00-20:55 每 5 分钟；时间窗口: 整点发送合并状态摘要，:30 再发送一次状态摘要；同轮心跳/健康检查并入摘要，异常巡检告警仍即时单发。状态摘要在 :00 / :30 合并同轮心跳和健康检查结果；若状态摘要关闭，整点 ok 心跳才会回退到 health_check_notify_enabled；warning / error 仍受 inspection_notify_enabled 控制。'),
  ...
  cfg('pb_cron_system_market_open_reminder_enabled', 'TRUE', 'TRUE', '开盘前状态提醒', 'Scheduler 调度(兼容 key)', 139, '每日 09:20 发送开盘前系统状态；若为周末或未检测到交易计划，则改发闭市/休市提醒。Cron: */5 * * * *；周期: 每 5 分钟轮询一次；时间窗口: 内部按 ET 09:20 仅发送一次。受 Scheduler 总开关、本开关和 status_notify_enabled 共同控制。'),
