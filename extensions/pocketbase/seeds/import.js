...
const configData = [
  ...
  cfg('exit_policy_profile', 'fixed_atr_rr', 'fixed_atr_rr', '退出策略 Profile', '信号与反转', 222.25, 'fixed_atr_rr=固定 ATR/RR 初始 bracket + ATR 收紧；signal_mode_adaptive_v1=按信号模式路由不同 TP/SL 策略；signal_mode_adaptive_v2=候选状态化止盈，MR 固定目标、trend/breakout 用 runner/checkpoint'),
  ...
  cfg('intraday_include_legacy_signals', 'false', 'false', '日内策略包含旧 MR 信号', '信号与反转', 222.697, 'intraday_sd_v1 默认只生成 SD squeeze / VWAP 回踩新 setup；如需临时回退混合旧 MR 信号，可设为 true'),
