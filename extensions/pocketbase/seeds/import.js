...
const configData = [
  ...
  cfg('pb_cron_ibkr_weekly_reauth_reminder_enabled', 'TRUE', 'TRUE', '周验证提醒', 'PB Cron 调度', 137, '每周发送一张周验证提醒卡片；只提醒，不自动触发 Gateway 登录。Cron: 0 5 * * 1；周期: 每周一 UTC 05:00；时间窗口: 北京时间周一 13:00 / 美东周一 01:00(EDT) 或 00:00(EST)。受 PB 调度总开关和本开关共同控制。'),
  cfg('pb_cron_ibkr_weekly_reauth_followup_enabled', 'TRUE', 'TRUE', '周验证补提醒', 'PB Cron 调度', 137.5, '若周验证仍停在待手动触发阶段，则补发一张飞书验证卡片提醒。Cron: 30 7 * * 1；周期: 每周一 UTC 07:30；时间窗口: 北京时间周一 15:30 / 美东周一 03:30(EDT) 或 02:30(EST)。受 PB 调度总开关和本开关共同控制。'),
  cfg('pb_cron_ibkr_2fa_hourly_check_enabled', 'TRUE', 'TRUE', '2FA 每小时提醒', 'PB Cron 调度', 138, '若 2FA 仍未恢复，则按小时补发飞书验证卡片提醒。Cron: 5 4-20 * * 1-5；周期: 工作日 UTC 每小时 05 分；时间窗口: 盘前到盘后。受 PB 调度总开关和本开关共同控制。'),
  cfg('pb_cron_system_market_open_reminder_enabled', 'TRUE', 'TRUE', '开盘前状态提醒', 'PB Cron 调度', 139, '每日 09:20 发送开盘前系统状态；若为周末或未检测到交易计划，则改发闭市/休市提醒。Cron: */5 * * * *；周期: 每 5 分钟轮询一次；时间窗口: 内部按 ET 09:20 仅发送一次。受 PB 调度总开关、本开关和 status_notify_enabled 共同控制。'),
