...
const configData = [
  ...
  cfg('pb_cron_ibkr_scan_runtime_enabled', 'TRUE', 'TRUE', '信号窗口日筛', 'Scheduler 调度(兼容 key)', 133, '按 09:20 ET 触发信号窗口 daily scan，刷新当天 candidate / active 目标池。Cron: 20 9 * * 1-5；时区: America/New_York；周期: 美东工作日 09:20；时间窗口: 09:20 ET 信号窗口日筛。受 Scheduler 总开关、Compute 开关和本开关共同控制。'),
  ...
  cfg('pb_cron_ibkr_early_expansion_topup_enabled', 'TRUE', 'TRUE', '信号窗口入池补充', 'Scheduler 调度(兼容 key)', 133.2, '09:20 主池后，09:29 ET 先做开盘预采集，并在 09:30-10:30 ET 每 10 分钟执行增量扩池；只增加新可操作标的并按需提醒。Cron: 29,30,40,50 9 * * 1-5 与 0,10,20,30 10 * * 1-5；时区: America/New_York。受 Scheduler 总开关、本开关和 status_notify_enabled 共同控制。'),
  cfg('pb_cron_ibkr_intraday_window_admission_enabled', 'TRUE', 'TRUE', '信号窗口入池', 'Scheduler 调度(兼容 key)', 133.25, '扫描 trade 观察池中已有新鲜 5m bars 的非 active 标的，按窗口与质量门槛自动加入 active 目标池。Cron: */5 9-15 * * 1-5；时区: America/New_York。'),
  ...
  cfg('intraday_min_rvol_20', '1.2', '1.2', '日内最小 RVOL20', '信号与反转', 222.65, 'intraday_sd_v1 过滤：rvol_20 低于该值时不生成突破/回踩新 setup；少而精默认 1.2，0 表示关闭'),
  cfg('intraday_min_atr_pct', '0.08', '0.08', '日内最小 ATR%', '信号与反转', 222.66, 'intraday_sd_v1 过滤：atr_pct 低于该值时不生成突破/回踩新 setup；0 表示关闭'),
  cfg('intraday_max_atr_pct', '1.20', '1.20', '日内最大 ATR%', '信号与反转', 222.67, 'intraday_sd_v1 过滤：atr_pct 高于该值时不生成突破/回踩新 setup；0 表示关闭'),
  cfg('intraday_max_directional_day_change_pct', '4.0', '4.0', '日内顺方向涨跌幅上限', '信号与反转', 222.68, 'intraday_sd_v1 过滤：多头用 day_change_pct、空头用 -day_change_pct，超过该百分比时不追单；0 表示关闭'),
  ...
  cfg('ibkr_require_target_direction_alignment', 'true', 'true', '要求目标方向一致', '信号与反转', 222.698, '开启后 compute 信号下单前必须与当日 active target 的 direction_bias 一致；neutral 或缺失会阻止交易'),
  ...
  cfg('intraday_entry_window_end_time', '10:30', '10:30', '日内策略截止时间', '信号与反转', 222.9, 'intraday_sd_v1 新 setup 的最晚生成时间，ET；默认少而精窗口 09:35-10:30'),
  ...
  cfg('ibkr_order_flow_enabled', 'TRUE', 'TRUE', '订单流观察开关', '信号与反转', 225.0, '默认开启；启动后 paper/live 使用同一套订单流 candidate queue / execution pool / CVD 链路'),
  ...
  cfg('ibkr_daily_scan_time_et', '09:20', '09:20', '日筛触发时间', '标的订阅', 612, '每日自动初筛触发时间，ET 时区；当前设计为 09:20 信号窗口初筛'),
  ...
  cfg('ibkr_open_report_target_wait_sec', '45', '45', '开盘摘要等待补池秒数', '标的订阅', 619.74, '开盘摘要等待 09:29/open-report topup 完成的最大秒数；超时后仍发送摘要并标注开盘采集状态。'),
