...
在远端执行:
  ...
  PYTHONPATH=/opt/ibkr_compute/src python3 /opt/ibkr_compute/ops/monitor/e2e_test.py
"""
...
'''
    ts = int(time.time())
    os.makedirs("/opt/pocketbase/extensions/migrations", exist_ok=True)
    migration_path = "/opt/pocketbase/extensions/migrations/%d_created_ibkr_collections.js" % ts
...
def main():
    ...
    if gw_ok:
        ...
        print("  Run: source /opt/ibkr_compute/.env && PYTHONPATH=/opt/ibkr_compute/src python3 /opt/ibkr_compute/ops/auth/ibkr_login.py")
    print()
