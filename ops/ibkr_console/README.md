...
- `check_console_static_sync.py` now validates that `runtime/ibkr_console/static` and `runtime/pocketbase/pb_public` stay intentionally decoupled; PB keeps only its own landing page, not the old `common.js` / `assets/**` bundle or legacy console HTML shims.
