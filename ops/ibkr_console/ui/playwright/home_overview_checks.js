const HOME_STACK_REQUIRED_ROWS = ['TV Webhook', 'IBKR Exec', 'Runtime', 'Supporting Ops', 'Storage'];
...
async function waitForHomeOverviewReady(page, timeoutMs) {
  await page.waitForFunction(() => {
    ...
    return (
      ...
      document.querySelectorAll('#homeQuickLinks .home-tool-chip').length >= 2 &&
...
async function collectHomeOverviewIssues(page, mobile = false) {
  return page.evaluate(({ isMobileViewport, requiredRows }) => {
    ...
    if (stackSummary && !stackSummaryText.includes('TV webhook / IBKR execution / runtime / supporting ops')) {
    ...
    if (positionsLabel && !/(当前持仓|推断持仓|无实际持仓|持仓未取样|持仓接口不可用)/.test(positionsLabelText)) {
