...
INSTALL_MODE="auto"
DEFAULT_WORKSPACE="$AI_ASSISTANT_ROOT/ops/ibkr_console/ui/playwright"
NPM_CACHE_DIR="${NPM_CONFIG_CACHE:-${npm_config_cache:-${TMPDIR:-/tmp}/ai_assistant_npm_cache}}"

usage() {
  cat <<EOF
Usage: run_console_playwright_smoke.sh [options] [-- <pb_smoke.js args...>]
...
Common pb_smoke.js args:
  -- --desktop-only
  -- --mobile-only
  -- --target 'https://console.example.com/ibkr_indicators.html?environment=live'
  -- --target 'https://console.example.com/ibkr_indicators.html?environment=live' --with-indicator-trace
  -- --skip-indicator-trace
  -- --target 'https://console.example.com/ibkr_chart.html?environment=live' --with-wheel-scroll
  -- --skip-wheel-scroll

Notes:
  - set CONSOLE_BASE_URL to target the split console instead of legacy PocketBase page hosting
  - set API_BASE_URL or IBKR_API_PUBLIC_URL to preflight the control-plane /health endpoint before the browser smoke
  ...
  - see 'node ops/ibkr_console/ui/playwright/pb_smoke.js --help' for the full smoke CLI
EOF
}
...
API_HEALTH_BASE="${API_BASE_URL:-${IBKR_API_PUBLIC_URL:-}}"
if [[ -n "$API_HEALTH_BASE" ]]; then
  python3 - "$API_HEALTH_BASE" <<'PY'
import json
import sys
...
with urllib.request.urlopen(url, timeout=10) as response:
    ...
    if not (200 <= status < 300):
        raise SystemExit(f"API health check failed: {url} status={status}")
    body = response.read(65536).decode("utf-8", errors="replace")
    try:
        payload = json.loads(body)
    except Exception:
        payload = {"raw": body[:200]}
    print(f"[playwright-smoke] api health ok: {url} profile={payload.get('service_profile', '--')}")
PY
fi
