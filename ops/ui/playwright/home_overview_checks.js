const HOME_STACK_REQUIRED_ROWS = ['Runtime', 'Compute', 'PocketBase'];
...
async function waitForHomeOverviewReady(page, timeoutMs) {
  await page.waitForFunction(() => {
    ...
    return (
      ...
      document.querySelectorAll('#overviewServiceRows .home-stack-service').length >= 3
    );
  }, { timeout: timeoutMs });
}
...
async function collectHomeOverviewIssues(page, mobile = false) {
  return page.evaluate(({ isMobileViewport, requiredRows }) => {
    ...
    if (stackSummary && !stackSummaryText.includes('Runtime / Compute 已拆分')) {
      issues.push(`overview_stack_summary_text:${stackSummaryText || 'empty'}`);
    }
    if (stackRows.length < 3) issues.push(`overview_stack_row_count:${stackRows.length}`);
    requiredRows.forEach((label) => {
      if (!stackRowNames.includes(label)) {
        issues.push(`overview_stack_missing_row:${label}`);
      }
    });
    if (stackRowStatuses.length < 3) issues.push(`overview_stack_status_count:${stackRowStatuses.length}`);
