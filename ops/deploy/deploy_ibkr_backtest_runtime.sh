...
IBKR_REMOTE_ROOT="$IBKR_BACKTEST_REMOTE_ROOT"
SYSTEMD_DIR="/etc/systemd/system"
VENV_DIR="$IBKR_BACKTEST_REMOTE_ROOT/venv"
OPS_REMOTE_ROOT="$IBKR_BACKTEST_REMOTE_ROOT/ops"
...
usage() {
  cat <<EOF
Usage: deploy_ibkr_backtest_runtime.sh [options]
...
Options:
  ...
  --status-only         Show ibkr-backtest status and exit
...
if [[ "$STATUS_ONLY" -eq 1 ]]; then
  show_remote_systemd_statuses ibkr-backtest
  exit 0
fi

prepare_target_plan ibkr_backtest

if [[ "$NO_MANAGED_CHANGES" -eq 1 ]]; then
  deploy_log "No managed changes for ibkr-backtest."
  exit 0
fi

if [[ "$PLAN_ONLY" -eq 1 ]]; then
  print_deploy_plan ibkr_backtest
  exit 0
fi
...
case "$FINAL_MODE" in
  scope)
    ...
    ;;
  files)
    files_deploy_paths_for_target ibkr_backtest "${PLAN_FILES[@]}"
    ;;
  package)
